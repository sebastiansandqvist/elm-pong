...
import Playground exposing (black, blue, game, move, moveX, moveY, rectangle, white, words)
import Set
...
updatePlayers keyboard pong =
    let
        ...
        isPressingW =
            Set.member "w" keyboard.keys

        isPressingS =
            Set.member "s" keyboard.keys
...
getPaddleCollisionDx pong =
    let
        isCollidingRightX =
            abs (pong.ballX - rightPaddleX) < paddleWidth / 2 + ballSize / 2

        isCollidingLeftX =
            abs (pong.ballX - leftPaddleX) < paddleWidth / 2 + ballSize / 2

        isCollidingRightY =
            abs (pong.ballY - pong.rightY) < paddleHeight / 2 + ballSize / 2

        isCollidingLeftY =
            abs (pong.ballY - pong.leftY) < paddleHeight / 2 + ballSize / 2
    in
...
handleGoal pong =
    let
        offscreenRight =
            pong.ballX - ballSize / 2 > gameWidth / 2

        offscreenLeft =
            pong.ballX + ballSize / 2 < -gameWidth / 2

        totalScore =
            pong.scoreL + pong.scoreR
    in
