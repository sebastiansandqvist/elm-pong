...
updatePlayers keyboard pong =
  let
    ...
    newLeftY = if isPressingW then pong.leftY + paddleSpeed else if isPressingS then pong.leftY - paddleSpeed else pong.leftY
    newRightY = if isPressingUp then pong.rightY + paddleSpeed else if isPressingDown then pong.rightY - paddleSpeed else pong.rightY
    clampUpper = (gameHeight / 2 - paddleHeight / 2)
    clampLower =  (-gameHeight / 2 + paddleHeight / 2)
  in
  {
    pong | leftY = clamp clampLower clampUpper newLeftY, rightY = clamp clampLower clampUpper newRightY
  }


getVerticalCollisionDy pong =
  if pong.ballY < (-gameHeight / 2 + ballSize / 2) then 1
  else if pong.ballY > (gameHeight / 2 - ballSize /2) then -1
  else if pong.ballDy > 0 then 1 else -1
...
getPaddleCollisionDx pong =
  ...
  in
  if isCollidingRightX && isCollidingRightY
  then -(abs pong.ballSpeed) - 0.5
  else if isCollidingLeftX && isCollidingLeftY
  then abs pong.ballSpeed + 0.5
  else pong.ballDx
